...
struct RestClientTests {
  ...
  func succeedGET(_ provider: Provider) async throws {
    let task: APITask<EchoResponse> = try provider.provider.call(.getRequest)
    let response = try await task.execute()
  ...
  func succeedPOST(_ provider: Provider) async throws {
    let task: APITask<EchoResponse> = try provider.provider.call(.postRequest)
    let response = try await task.execute()
