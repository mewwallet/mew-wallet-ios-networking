...
public extension MW {
  final class WebSocket: Sendable {
    ...
    private let consumers = ThreadSafe<[MW.WebSocket.Consumer]>([])
    ...
    public convenience init(
      ...
      configuration: MW.WebSocket.Configuration = .default
    ) throws {
      // Options
      let webSocketOptions = NWProtocolWebSocket.Options(.version13)
      let connectivityOptions = NWProtocolWebSocket.Options(.version13)
      if !headers.isEmpty {
    ...
    private func _process(state: NWConnection.State) -> Event? {
      ...
      case .waiting(let error):
        self._pendingRequests.write { pendings in
          pendings.forEach { $0.value.resume(throwing: MW.WebSocket.ConnectionError.notReachable) }
          pendings.removeAll()
        }
        self._change(state: .pending, from: nil)
